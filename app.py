...
model_directory = r"Neuroscanaimodel.h5"
...
brain_icon = Image.open(r"NeuroScanAI_Logo.png")
...
if uploaded_image is not None:
    ...
    if st.button("Expert Option"):
        ...
        # Sort percentages and tumor types based on percentages in descending order
        sorted_percentages, sorted_tumor_types = zip(*sorted(zip(percentages, tumor_types), reverse=True))

        # Display the confidence scores with progress bars
        for i, (tumor_type, percentage) in enumerate(zip(sorted_tumor_types, sorted_percentages)):
            st.write(f"{tumor_type}: {percentage}%")
            progress_bar = st.progress(int(percentage))
            # Set the color of the progress bar to green (optional, as Streamlit's default is green)
